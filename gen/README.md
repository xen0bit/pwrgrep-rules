...
**Frozen.** This produced the corpus and is kept as provenance; it is not run
any more. A rule is edited in place in `../rules`, and the counts and scores
below describe the translation as it stood rather than the corpus as it is. The
check that runs now is `../tools/validate.py`.

Most of `../rules` was generated. `port.jq` reads one YAML rule file and writes
...
At the time it ran, nothing under `../rules` except `../rules/pwrq` was edited
by hand: a fix went in `port.jq` and the corpus was regenerated. Those eighteen
hand-written rules now sit in the category they search rather than in a
