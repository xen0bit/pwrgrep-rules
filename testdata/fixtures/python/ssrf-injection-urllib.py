# Django urlopen with request data as the URL (SSRF).
#
...
def view_a(request):
    ...
    urllib.request.urlopen(request.GET.get("url"))
    return 1
...
def view_literal(request):
    # ok: ssrf-injection-urllib
    urllib.request.urlopen("https://api.x.com/health")
    return 1
